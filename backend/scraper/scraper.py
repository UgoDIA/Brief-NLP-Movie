...
def insert_into_database(id_film, title, ranking, presse_score, spectateurs_score):
    ...
    cursor.execute(insert_query, (id_film, title, ranking, presse_score, spectateurs_score))
    connection.commit()
    cursor.close()
    connection.close()
...
def scrape_movies_to_db(url):
    ...
    for film in films:
        ...
        insert_into_database(id_film, title.text.strip(), ranking.text.strip(), presse_score.text.strip(), spectateurs_score.text.strip())
